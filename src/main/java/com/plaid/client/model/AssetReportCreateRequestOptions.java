...
public class AssetReportCreateRequestOptions {
  ...
  **/
  @javax.annotation.Nullable
  @ApiModelProperty(value = "This field can be used to add additional options for the Asset Report. To fetch `investments` data (transactions, holdings, etc.) in the Asset Report, `investments` must be specified in `add_ons`. For Fast Assets, `fast_assets` must be specified in `add_ons`.")
