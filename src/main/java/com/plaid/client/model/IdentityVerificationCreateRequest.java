...
public class IdentityVerificationCreateRequest {
  ...
  **/
  ...
  public IdentityVerificationCreateRequest userId(String userId) {
  ...
  **/
  @javax.annotation.Nullable
  @ApiModelProperty(example = "usr_dddAs9ewdcDQQQ", value = "Unique user identifier, created by calling `/user/create`. Either a `user_id` or the `client_user_id` must be provided. The `user_id` may only be used instead of the `client_user_id` if you were not a pre-existing user of `/user/create` as of December 10, 2025, or if you have since [migrated to the new User APIs](https://plaid.com/docs/api/users/migrate-to-new-user-apis); for more details, see [New User APIs](https://plaid.com/docs/api/users/user-apis). If both this field and a `client_user_id` are present in a request, the `user_id` must have been created from the provided `client_user_id`.")
  ...
  **/
  @javax.annotation.Nullable
  @ApiModelProperty(example = "true", value = "An optional flag specifying how you would like Plaid to handle attempts to create an Identity Verification when an Identity Verification already exists for the provided `client_user_id` and/or `user_id`, and `template_id`. If idempotency is enabled, Plaid will return the existing Identity Verification. If idempotency is disabled, Plaid will reject the request with a `400 Bad Request` status code if an Identity Verification already exists for the supplied `client_user_id` and/or `user_id`, and `template_id`.")
  ...
  public boolean equals(Object o) {
    ...
    return Objects.equals(this.clientUserId, identityVerificationCreateRequest.clientUserId) &&
        Objects.equals(this.userId, identityVerificationCreateRequest.userId) &&
  ...
  @Override
  public int hashCode() {
    return Objects.hash(clientUserId, userId, isShareable, templateId, gaveConsent, user, clientId, secret, isIdempotent);
  }
